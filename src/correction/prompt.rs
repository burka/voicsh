...
/// Build a prompt with confidence-annotated tokens for T5 correction.
///
/// Formats each token with its probability: `word[0.85] another[0.92]`
/// This helps the model identify which words are likely wrong.
pub fn build_confidence_prompt(tokens: &[TokenProbability]) -> String {
    if tokens.is_empty() {
        return String::new();
    }
    let mut parts = Vec::with_capacity(tokens.len());
    for tp in tokens {
        let word = tp.token.trim();
        if word.is_empty() {
            continue;
        }
        parts.push(format!("{word}[{:.2}]", tp.probability));
    }
    parts.join(" ")
}

/// Strip confidence score annotations `[0.XX]` from text.
///
/// Used to clean T5 output that might echo back confidence scores.
pub fn strip_confidence_scores(text: &str) -> String {
    ...
    // Collapse consecutive spaces from removed scores
...
mod tests {
    ...
    #[test]
    fn build_confidence_prompt_formats_tokens() {
        let tokens = vec![
            TokenProbability {
                token: "the".into(),
                probability: 0.95,
        ...
        assert_eq!(
            build_confidence_prompt(&tokens),
            "the[0.95] quik[0.30] brown[0.92]"
        );
    }

    #[test]
    fn build_confidence_prompt_empty_returns_empty() {
        assert_eq!(build_confidence_prompt(&[]), "");
    }

    #[test]
    fn build_confidence_prompt_skips_whitespace_only_tokens() {
        let tokens = vec![
            TokenProbability {
                token: "hello".into(),
                probability: 0.90,
        ...
        assert_eq!(build_confidence_prompt(&tokens), "hello[0.90] world[0.80]");
    }

    #[test]
    fn strip_confidence_scores_removes_scores() {
        assert_eq!(
            strip_confidence_scores("hello[0.95] world[0.30]"),
            "hello world"
        );
    }

    #[test]
    fn strip_confidence_scores_preserves_non_score_brackets() {
        assert_eq!(
            strip_confidence_scores("hello [world] test"),
            "hello [world] test"
        );
    }

    #[test]
    fn strip_confidence_scores_plain_text_unchanged() {
        assert_eq!(strip_confidence_scores("hello world"), "hello world");
    }

    #[test]
    fn strip_confidence_scores_empty_returns_empty() {
        assert_eq!(strip_confidence_scores(""), "");
    }
}
